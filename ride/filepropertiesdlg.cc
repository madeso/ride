...
class StringIntConverter {
 ...
 public:
  ...
  const wxArrayString& strings() const { return strings_; }

  int GetInt(int index) const { return ints_[index]; }
...
FilePropertiesDlg::FilePropertiesDlg(FileEdit* parent, wxStyledTextCtrl* ctrl)
    : ::ui::FileProperties(parent, wxID_ANY), ctrl_(ctrl) {
...
#else
  wxSingleChoiceDialog dlg(this, "Select new text encoding", "Text encoding",
                           CodePageString().strings());
...
void FilePropertiesDlg::OnChangeLineEnding(wxCommandEvent& event) {
  wxSingleChoiceDialog dlg(this, "Select new line ending", "Line ending?",
                           EOLString().strings());
