...
public:
  CppLanguage() : Language(_("C++"), wxSTC_LEX_CPP){
    ...
    const KeywordBuilder temp = KeywordBuilder()
      ...
      <<"reinterpret_cast"<<"return"<<"short"<<"signed"<<"sizeof"<<"static"<<"statistyle_cast"
...
#define wxSTC_RUST_WORD7 12
    */
    ...
    SetProp(text, wxT("fold.compact"), b2s01(settings.foldcompact()));
    SetProp(text, wxT("fold.at.else"), b2s01(settings.fold_at_else()));
    SetProp(text, wxT("fold.rust.syntax.based"), b2s01(settings.fold_cpp_syntax_based()));
    SetProp(text, wxT("fold.rust.comment.multiline"), b2s01(settings.fold_cpp_comment_multiline()));
    SetProp(text, wxT("fold.rust.comment.explicit"), b2s01(settings.fold_cpp_comment_explicit()));
    SetProp(text, wxT("fold.rust.explicit.anywhere"), b2s01(settings.fold_cpp_explicit_anywhere()));
...
public:
  ...
  void DoStyleDocument(wxStyledTextCtrl* text, const ride::Settings& settings) {
    ...
    SetKeys(text, 0, KeywordBuilder()
      << "message" << "required" << "optional" << "repeated" << "true" << "false"
      << "enum" << "import" << "group" << "extensions" << "to" << "max" << "oneof"
      << "package" << "service" << "rpc" << "returns");
    SetKeys(text, 1, KeywordBuilder()
      << "double" << "float" << "int32" << "int64" << "uint32" << "uint64"
      << "sin32" << "sint64" << "fixed32" << "fixed64" << "sfixed32" << "sfixed64"
      << "bool" << "string" << "bytes"); // secondary
...
public:
  ...
  void DoStyleDocument(wxStyledTextCtrl* text, const ride::Settings& settings) {
    ...
    SetStyle(text, wxSTC_PROPS_DEFAULT, settings.fonts_and_colors().default_style());
...
public:
  ...
  void DoStyleDocument(wxStyledTextCtrl* text, const ride::Settings& settings) {
    ...
    SetStyle(text, wxSTC_CMAKE_DEFAULT, settings.fonts_and_colors().default_style());
...
public:
  ...
  void DoStyleDocument(wxStyledTextCtrl* text, const ride::Settings& settings) {
    ...
    SetStyle(text, wxSTC_LUA_DEFAULT, settings.fonts_and_colors().default_style());
...
public:
  ...
  void DoStyleDocument(wxStyledTextCtrl* text, const ride::Settings& settings) {
    ...
    SetStyle(text, wxSTC_YAML_DEFAULT, settings.fonts_and_colors().default_style());
