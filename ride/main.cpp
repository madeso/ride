...
class EditCtrl : public wxControl {
public:
  EditCtrl(wxAuiNotebook* anotebook, wxWindow* parent) : wxControl(parent, wxID_ANY), notebook(anotebook) {
    ...
    text->Connect(wxEVT_STC_MARGINCLICK, wxStyledTextEventHandler(EditCtrl::OnMarginClick), NULL, this);
...
wxBEGIN_EVENT_TABLE(EditCtrl, wxControl)
wxEND_EVENT_TABLE()
...
void MainWindow::OnNotebookPageClose(wxAuiNotebookEvent& evt) {
  ...
  if (window->IsKindOf(CLASSINFO(EditCtrl))) {
    EditCtrl* edit = reinterpret_cast<EditCtrl*>(window);
