...
void AddAllOutput(const wxString& result, const CompilerMessage::Source source, const wxString folder, MainWindow* main_window) {
...
void CompileProtoFile(const wxString& full_path, MainWindow* main_window) {
  ...
  if (proto_compile_successful) {
    main_window->compiler_output().Append(wxString::Format("%s compiled without errors\n", full_path));
  }
  else {
