...
namespace ride::backends::sdl
{
    ...
    {
        bool loaded = false;
        std::shared_ptr<api::Texture> texture;
        ...
        bool load_single_glyphset_or_fail(LoadedFontData* font, int idx, int width, int height,
                                          const api::Platform& loader);
        void load_glyphset(LoadedFontData* font, int idx, const api::Platform& loader);
    };
    ...
    struct Font : api::Font
    {
        ...
        bool load_font(const std::string& filename, Px size, api::Platform* app) override;

        bool load_font(const embedded_binary& data, Px size, api::Platform* app);
        ...
        Px draw(api::Renderer* rend, const std::string& text, Px x, Px y, Color color) override;
    };
