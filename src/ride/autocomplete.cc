   ...
   */
	...
	for (int y = 0; y < h; ++y)
	{
		for (int x = 0; x < w; ++x)
		{
			...
			pixels[index + 3] = img.HasAlpha() ? img.GetAlpha(x, y)
...
)
{
	...
	const wxString cmd = wxString::Format(
		(machine.racer + " complete %d %d \"%s\"").c_str(), linenum, charnum, path
	);
	CmdRunner::Run(root_folder, cmd, CollectRideSpecificEnviroment(machine), &output);
	// parse output
...
)
{
	...
	const bool is_space_before
		= text->GetRange(start_position - 1, start_position).Trim(true).Trim(false).IsEmpty();
		...
		|| (text->AutoCompActive() == false && length >= word_wait_chars))
	{
		...
		if (is_space_before)
		{
			// TODO(Gustav) allow user to save theese templates
