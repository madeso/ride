...
void Group::set_title(const wxString& title)
{
	title_ = title;
}

const wxString& Group::title() const
{
	return title_;
}
...
const Item& Group::GetItem(int i) const
{
	return items_[i];
}
...
)
{
	...
	if (draw_image)
	{
		...
		{
			dc->DrawBitmap(bitmap, x, rect.y + (rect.height - bitmap.GetHeight()) / 2, true);
		}
...
void ExtendSize(wxDC* dc, wxFont font, wxString title, const Style& style, wxSize* sz)
{
	dc->SetFont(font);
