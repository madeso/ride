...
{
	for (size_t group_index = group; group_index <= items.GetGroupCount(); ++group_index)
	{
...
{
	for (int group_index = items.GetGroupCount() - 1; group_index >= 0; --group_index)
	{
...
void Ctrl::set_items(const ItemList& items)
{
	items_ = items;
}

const ItemList& Ctrl::items() const
{
	return items_;
}
...
Index Ctrl::selection() const
{
	return selection_;
}

void Ctrl::OnPaint(wxPaintEvent& WXUNUSED(event))
{  // NOLINT
...
void Ctrl::OnChar(wxKeyEvent& WXUNUSED(event))
{
}  // NOLINT
...
void Ctrl::OnEraseBackground(wxEraseEvent& WXUNUSED(event))
{  // NOLINT
	// Do nothing
}

wxSize Ctrl::DoGetBestClientSize() const
{
...
public:
	...
	LayoutCalculator(wxSize is, const Style& style)
		: style_(style)
	{
		columnCount = 1;
	...
	wxRect rect() const
	{
		return wxRect(x, y, itemSize.x, itemSize.y);
	}
	...
	const wxSize& overall_size() const
	{
		return overall_size_;
	}
...
void Ctrl::CalculateLayout(wxDC& dc)
{  // NOLINT
