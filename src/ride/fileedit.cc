...
void FileEdit::Cut()
{
	if (text_->GetReadOnly() || (text_->GetSelectionEnd() - text_->GetSelectionStart() <= 0))
		return;
	text_->Cut();
	UpdateTitle();
}
...
void FileEdit::GotoLine()
{
	...
	const auto new_line_one_based = wxGetNumberFromUser(
		message, wxEmptyString, "Goto line", text_->GetCurrentLine() + 1, 1, total_plus_one
	);
...
void FileEdit::OpenInOnlineDocumentation()
{
	...
	const wxString url_to_open
		= wxString::Format("http://doc.rust-lang.org/std/?search=%s", current_selection_or_word);
...
bool FileEdit::AcceptsFocus() const
{
	return true;
}

bool FileEdit::AcceptsFocusFromKeyboard() const
{
	return true;
}
...
public:
	...
	explicit StyledTextCtrl(FileEdit* parent)
		: wxStyledTextCtrl(
			  parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxBORDER_NONE | wxVSCROLL
		  )
		, parent_(parent)
	{
	...
	{  // NOLINT
		...
		if (lastKeyDownConsumed && evt.GetUnicodeKey() > 255) lastKeyDownConsumed = false;
	...
	void ProcessCharEvent(wxKeyEvent& evt)
	{  // NOLINT
		assert(parent_);
	...
	{  // NOLINT
		assert(parent_);
		if (parent_->ProcessKey(
				static_cast<wxKeyCode>(evt.GetKeyCode()),
...
public:
	...
	explicit UndoActionRaii(wxStyledTextCtrl* text)
		: text_(text)
	{
...
StringType IsStringAt(wxStyledTextCtrl* text, int pos, bool* support_language = nullptr)
{
	...
	case wxSTC_LEX_CPP:
		...
		switch (style)
		{
		case wxSTC_C_STRING: return StringType::DOUBLE_QUOTE;
		case wxSTC_C_CHARACTER: return StringType::SINGLE_QUOTE;
		}
		break;
	...
	case wxSTC_LEX_RUST:
		...
		switch (style)
		{
		case wxSTC_RUST_STRING: return StringType::DOUBLE_QUOTE;
		case wxSTC_RUST_STRINGR:
			// TODO(Gustav): Fix raw string literals
			return StringType::DOUBLE_QUOTE;
		case wxSTC_RUST_CHARACTER: return StringType::SINGLE_QUOTE;
		}
		break;
#endif
	}
...
bool HandleParaEditCommon(
	int tab_width,
	ride::AutoComplete ac_setting,
	wxChar c,
...
bool HandleParaEditQuote(
	char c,
	ride::AutoComplete ac,
	wxStyledTextCtrl* text,
...
bool FileEdit::ProcessCharEvent(wxChar c)
{
	...
	if (HandleParaEditCommon(
			tab_width, main_->settings().autocomplete_parentheses, c, text_, false, '(', ')'
		))
		return true;
	if (HandleParaEditCommon(
			tab_width, main_->settings().autocomplete_curly_braces, c, text_, true, '{', '}'
		))
		return true;
	if (HandleParaEditCommon(
			tab_width, main_->settings().autocomplete_brackets, c, text_, false, '[', ']'
		))
		return true;

	if (HandleParaEditQuote(
			c,
			main_->settings().autocomplete_singlequote,
			text_,
			SINGLE_QUOTE,
	...
	if (HandleParaEditQuote(
			c,
			main_->settings().autocomplete_doublequote,
			text_,
			DOUBLE_QUOTE,
...
)
{
	...
	case ride::HES_BASIC:
		if (right)
		{
			...
			{
			case SelectionState::NONE: text_->LineEnd(); break;
			case SelectionState::STREAM: text_->LineEndExtend(); break;
			case SelectionState::RECT: text_->LineEndRectExtend(); break;
		...
		else
		{
			switch (selection_state)
			{
			case SelectionState::NONE: text_->Home(); break;
			case SelectionState::STREAM: text_->HomeExtend(); break;
			case SelectionState::RECT: text_->HomeRectExtend(); break;
			default: assert(false && "Unhandled selection state");
			}
		}
		break;
	case ride::HES_DISPLAY:
		if (right)
		{
			switch (selection_state)
			{
			case SelectionState::NONE: text_->LineEndDisplay(); break;
			case SelectionState::STREAM: text_->LineEndDisplayExtend(); break;
			case SelectionState::RECT: text_->LineEndRectExtend(); break;
			default: assert(false && "Unhandled selection state");
			}
		}
		else
		{
			switch (selection_state)
			{
			case SelectionState::NONE: text_->HomeDisplay(); break;
			case SelectionState::STREAM: text_->HomeDisplayExtend(); break;
			case SelectionState::RECT: text_->HomeRectExtend(); break;
			default: assert(false && "Unhandled selection state");
			}
		}
	...
	case ride::HES_WRAP:
		if (right)
		{
			switch (selection_state)
			{
			case SelectionState::NONE: text_->LineEndWrap(); break;
			case SelectionState::STREAM: text_->LineEndWrapExtend(); break;
			case SelectionState::RECT: text_->LineEndRectExtend(); break;
			default: assert(false && "Unhandled selection state");
			}
		}
		else
		{
			switch (selection_state)
			{
			case SelectionState::NONE: text_->HomeWrap(); break;
			case SelectionState::STREAM: text_->HomeWrapExtend(); break;
			case SelectionState::RECT: text_->HomeRectExtend(); break;
			default: assert(false && "Unhandled selection state");
			}
		}
	...
	case ride::HES_VCBASIC:
		...
		else
		{
			switch (selection_state)
			{
			case SelectionState::NONE: text_->VCHome(); break;
			case SelectionState::STREAM: text_->VCHomeExtend(); break;
	...
	case ride::HES_VCWRAP:
		...
		else
		{
			switch (selection_state)
			{
			case SelectionState::NONE: text_->VCHomeWrap(); break;
			case SelectionState::STREAM: text_->VCHomeWrapExtend(); break;
			case SelectionState::RECT: text_->VCHomeRectExtend(); break;
			default: assert(false && "Unhandled selection state");
			}
		}
...
bool FileEdit::ProcessKey(wxKeyCode key, wxKeyModifier mod)
{
	...
	const wxKeyModifier without_shift = static_cast<wxKeyModifier>(mod & ~wxMOD_SHIFT);
	const bool is_shift_down = (mod & wxMOD_SHIFT) == wxMOD_SHIFT;

	const SelectionState selection_state
		= is_shift_down
			? (text_->SelectionIsRectangle() ? SelectionState::RECT : SelectionState::STREAM)
			: SelectionState::NONE;
	...
	)
	{
		...
		if (right)
		{
			switch (selection_state)
			{
			case SelectionState::NONE: text_->WordRight(); break;
			case SelectionState::STREAM:
			case SelectionState::RECT: text_->WordRightExtend(); break;
			default: assert(false && "Unhandled selection state");
			}
		}
		else
		{
			switch (selection_state)
			{
			case SelectionState::NONE: text_->WordLeft(); break;
			case SelectionState::STREAM:
			case SelectionState::RECT: text_->WordLeftExtend(); break;
			default: assert(false && "Unhandled selection state");
			}
		}
		return true;
	...
	if (without_shift == wxMOD_ALT && left_or_right)
	{
		...
		if (right)
		{
			switch (selection_state)
			{
			case SelectionState::NONE: text_->WordPartRight(); break;
			case SelectionState::STREAM:
			case SelectionState::RECT: text_->WordPartRightExtend(); break;
			default: assert(false && "Unhandled selection state");
			}
		}
		else
		{
			switch (selection_state)
			{
			case SelectionState::NONE: text_->WordPartLeft(); break;
			case SelectionState::STREAM:
			case SelectionState::RECT: text_->WordPartLeftExtend(); break;
			default: assert(false && "Unhandled selection state");
			}
		}
		return true;
	...
	)
	{
		...
		GoHomeEnd(text_, right, main_->settings().home_end_style, selection_state);
		return true;
	}
...
public:
	...
	explicit TrueFalse(bool* capture)
		: capture_(capture)
	{
		assert(capture_);
...
bool FileEdit::CanClose(bool can_abort)
{
	if (ShouldBeSaved())
	{
		...
		const wxString title_error
			= "\"" + CalculateDocumentName() + "\" has changed since last time, save it?";

		const DialogResult answer
			= can_abort
				? ShowYesNoCancel(
					  this, caption, yes_button, no_button, cancel_button, title_ok, title_error
				  )
...
void FileEdit::OnCharAdded(wxStyledTextEvent& event)
{
	...
	const wxString character_before_entered
		= text_->GetTextRange(text_->GetCurrentPos() - 2, text_->GetCurrentPos() - 1);
	...
	if (entered_character == '\n' || entered_character == '\r')
	{
		...
		// the actual width
		const int indentation_in_chars
			= main_->settings().useTabs ? indentation_in_tabs : indentation_in_spaces;
		...
		if (main_->settings().auto_indentation != ride::AUTOINDENTATION_NONE)
		{
			...
			if (current_line_content.StartsWith("}"))
			{
				...
				text_->SetLineIndentation(
					current_line + 1, indentation_in_spaces_ajdusted - main_->settings().tabWidth
				);
			}
		}
	}
			...
			&& current_start_position != current_end_position)
		{
			...
			if (is_keyword == false)
			{
				...
				while (true)
				{
					...
					int match_position = FindStcText(
						text_, search_point, text_->GetLength(), current_text, flags, nullptr
					);
					if (match_position == -1)
					{
						break;
...
void FileEdit::UpdateStatusText()
{
	...
	main_->SetStatusText(
		wxString::Format("Ch %d", cp - text_->PositionFromLine(line)), STATUSBAR_CH
	);
}
