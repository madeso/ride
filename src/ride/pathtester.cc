...
{
	wxString output;
	if (CmdRunner::Run("", app.c_str() + cmd, CollectRideSpecificEnviroment(machine), &output)
		== false)
	{
		return "";
	}
...
const wxString PathTester::cargo() const
{
	return cargo_;
}

const wxString PathTester::rustc() const
{
	return rustc_;
}

const wxString PathTester::racer() const
{
	return racer_;
}

const wxString PathTester::protoc() const
{
	return protoc_;
}
