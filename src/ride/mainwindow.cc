...
void MainWindow::SetupMenu()
{
	...
	if (tab && tab->ToFileEdit())
	{
		...
		menu_edit->AppendSeparator();
		AddMenuItem(
			menu_edit, ID_EDIT_TOUPPER, "Make UPPERCASE\tCtrl-Shift-U", "", edit_make_uppercase_xpm
		);
		AddMenuItem(
			menu_edit, ID_EDIT_TOLOWER, "Make lowercase\tCtrl-U", "", edit_make_lowercase_xpm
		);
		menu_edit->AppendSeparator();
		...
		menu_edit->AppendSeparator();
		AddMenuItem(menu_edit, ID_EDIT_SHOW_AUTOCOMPLETE, "Auto complete\tRawCtrl-Space", "");
		menu_edit->AppendSeparator();
		AddMenuItem(
			menu_edit,
	...
	AddMenuItem(
		...
		ID_PROJECT_SETTINGS,
		"Project settings...",
		"Change the ride project settings",
		project_settings_xpm
	);
	...
	AddMenuItem(
		menu_project, ID_PROJECT_CLEAN, "Clean", "Remove the target directory", project_clean_xpm
	);
	...
	AddMenuItem(
		menu_project, ID_REPLACE_IN_FILES, "Replace in files...\tCtrl-Shift-H", "Replace in files"
	);
	...
	menuItemViewProject_
		= AddMenuItem(menu_view, ID_VIEW_SHOW_PROJECT, "&Project pane\tAlt-0", "").Checkable();
	AddMenuItem(menu_view, ID_VIEW_SHOW_START, "&Start page", "");
	// build issues
	menuItemViewFind_
		= AddMenuItem(menu_view, ID_VIEW_SHOW_FINDRESULT, "Find &result pane\tAlt-2", "")
			  .Checkable();
	// app output
	menuItemViewBuild_
		= AddMenuItem(menu_view, ID_VIEW_SHOW_BUILD, "&Build pane\tAlt-4", "").Checkable();
	menuItemViewCompile_
		= AddMenuItem(menu_view, ID_VIEW_SHOW_COMPILE, "&Compile pane", "").Checkable();
...
void MainWindow::BindEvents()
{
	...
	Bind(
		wxEVT_MENU,
		&MainWindow::OnEditOpenInOnlineDocumentation,
		this,
		ID_EDIT_OPEN_IN_ONLINE_DOCUMENTATION
	);
	...
	for (int i = ID_SPECIAL_TAB_NO_EVENT + 1; i < ID_SPECIAL_TAB_LAST_EVENT; ++i)
	{
	...
	entries.push_back(wxAcceleratorEntry(wxACCEL_RAW_CTRL | wxACCEL_SHIFT, WXK_TAB, ID_TAB_PREV));
...
void MainWindow::SetStatusBarText(const wxString& text, StatusBarWidgets widget)
{
	assert(statusbar_);
...
// todo: set icon
	...
	aui_.AddPane(
		findres_window_,
		wxAuiPaneInfo().Name(PANE_FIND_1).Caption("Find result").Bottom().CloseButton(true)
	);
	...
	aui_.AddPane(
		project_explorer_,
		wxAuiPaneInfo().Name(PANE_PROJECT).Caption("Project").Left().CloseButton(true)
	);
...
std::vector<switcher::Item> ListPanes(wxAuiManager* aui)
{
	...
	for (size_t pane_index = 0; pane_index < aui->GetAllPanes().GetCount(); pane_index++)
	{
...
{
	static bool Check(wxToolBar* toolbar)
	{
...
void AddGroup(
	const std::vector<switcher::Item>& toolbars, switcher::ItemList* items, const wxString& title
)
{
...
void MainWindow::OnTab(bool forward)
{
	...
	switcher::Group& files = items.AddGroup(switcher::Group(_("Active Files")), &files_group_index);
	...
	const switcher::Index focus = vs_focus
									? switcher::Index(files_group_index, files.GetIndexForFocus())
	...
	style.text_margin_x_ = settings_.switcher_text_margin_x;
	style.text_margin_y_ = settings_.switcher_text_margin_y;
	style.x_margin_ = settings_.switcher_x_margin;
	style.y_margin_ = settings_.switcher_y_margin;
	style.row_spacing_ = settings_.switcher_row_spacing;
	style.col_spacing_ = settings_.switcher_col_spacing;
	style.item_maxwidth_ = settings_.switcher_item_maxwidth;
	style.item_maxheight_ = settings_.switcher_item_maxheight;
	style.dlg_main_border_ = settings_.switcher_dlg_main_border;
	style.dlg_item_border_ = settings_.switcher_dlg_item_border;
	style.min_width_ = settings_.switcher_min_width;
	style.min_height_ = settings_.switcher_min_height;
	...
	if (ans == wxID_OK && dlg.GetSelection() != SWITCHER_NOT_FOUND)
	{
		...
		{
			wxAuiNotebook* nb = wxDynamicCast(item.window()->GetParent(), wxAuiNotebook);
...
void MainWindow::OnNotebookNavigation(wxNavigationKeyEvent& nav)
{  // NOLINT
	OnTab(nav.GetDirection());
}

void MainWindow::OnTabNext(wxCommandEvent& event)
{
	OnTab(true);
}

void MainWindow::OnTabPrev(wxCommandEvent& event)
{
	OnTab(false);
}
...
const ride::MachineSettings& MainWindow::machine() const
{
	return machine_;
}
...
void MainWindow::ProjectSettingsHasChanged()
{
	UpdateAllEdits();
}
