...
public:
	...
	bool ToGui(TContainer* build, bool togui)
	{
		...
		if (togui)
		{
			...
			{
				uiBuildFeatures->AppendString(TContainerFunctions::GetDisplayString(build, i));
			}
	...
	bool Remove(TContainer* build)
	{
		...
		// one
		if (new_selection == -1 && TContainerFunctions::Size(build) > 0) new_selection = 0;
