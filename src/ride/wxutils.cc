...
public:
	...
	void next()
	{
		index_ = list_->GetNextItem(index_, geom_, state_);
	}
	...
	{
		return ! (list_ == o.list_ && geom_ == o.geom_ && state_ == o.state_ && index_ == o.index_);
	}
...
public:
	...
	ListCtrlIterator end()
	{
		return ListCtrlIterator(list_, geom_, state_, -1);
	}
...
DialogResult CDL(int dl)
{
	switch (dl)
	{
	case wxID_YES: return DialogResult::YES;
	case wxID_NO: return DialogResult::NO;
	case wxID_CANCEL: return DialogResult::CANCEL;
	default: assert(0 && "Invalid dialog result"); return DialogResult::CANCEL;
	}
}
...
int FindStcText(
	wxStyledTextCtrl* stc, int minPos, int maxPos, const wxString& text, int flags, int* findEnd
)
{
	assert(stc);
