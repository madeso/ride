...
void Pile::Redraw(wxDC& dc)
{
	...
	if (m_topCard >= 0)
	{
		if (m_dx == 0 && m_dy == 0)
		{
			if ((canvas)
				&& (MyIsExposed(
					canvas, m_x, m_y, (int) (Card::GetScale() * 60), (int) (Card::GetScale() * 200)
				)))
		...
		else
		{
			...
			for (int i = 0; i <= m_topCard; i++)
			{
				if ((canvas)
					&& (MyIsExposed(
						canvas, x, y, (int) (Card::GetScale() * 60), (int) (Card::GetScale() * 200)
					)))
...
Card* Pile::GetCard(int x, int y)
{
	...
	{
		if (x >= cardX && x <= cardX + Card::GetWidth() && y >= cardY
			&& y <= cardY + Card::GetHeight())
		{
			return m_cards[i];
		}
