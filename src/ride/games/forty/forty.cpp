...
void FortyFrame::SetupFortyMenu(wxMenuBar* menuBar)
{
	...
	gameMenu->Append(
		FORTY_HELP_CONTENTS,
		wxT("&Help Contents"),
		wxT("Displays information about playing the game")
	);
...
{
	const bool largecards = false;
	if (largecards) Card::SetScale(1.3);
...
FortyAboutDialog::FortyAboutDialog(
	wxWindow* parent,
	wxWindowID id,
...
bool FortyAboutDialog::AddControls(wxWindow* parent)
{
	...
	wxHtmlWindow* html
		= new wxHtmlWindow(this, ID_ABOUT_HTML_WINDOW, wxDefaultPosition, htmlSize, borderStyle);
	html->SetBorders(10);
