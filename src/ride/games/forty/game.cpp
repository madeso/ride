...
Game::Game(FortyFrame* frame, int wins, int games, int score)
	...
	, m_bmapCard(0)
{
	symbol = new wxBitmap(symbols_xpm);
	if (! symbol->IsOk())
	{
		::wxMessageBox(wxT("Failed to load bitmap CardSymbols"), wxT("Error"));
	}

	picture = new wxBitmap(Pictures);
	if (! picture->IsOk())
	{
	...
	{
		m_foundations[i]
			= new Foundation(frame, 2 + (i / 4) * (CardWidth + 2), 2 + (i % 4) * (CardHeight + 2));
	}
...
wxBitmap* Game::symbolBitmap()
{
	return symbol;
}

wxBitmap* Game::pictureBitmap()
{
	return picture;
}
...
void Game::Layout()
{
	...
	for (i = 0; i < 8; i++)
	{
		m_foundations[i]->SetPos(2 + (i / 4) * (CardWidth + 2), 2 + (i % 4) * (CardHeight + 2));
	}
...
void Game::DoMove(wxDC& dc, Pile* src, Pile* dest)
{
	...
	{
		wxMessageBox(
			wxT("Game::DoMove() Undo buffer full"), wxT("Debug message"), wxOK | wxICON_EXCLAMATION
		);
	}
	...
	if (HaveYouWon())
	{
		...
		if (wxMessageBox(
				wxT("Do you wish to play again?"),
				wxT("Well Done, You have won!"),
...
void Game::Redraw(wxDC& dc)
{
	...
	if (m_bmap == 0)
	{
		...
		memoryDC.SetBrush(BackgroundBrush());
...
{
	if (m_pack->GetCard(x, y) && m_pack->GetCard(x, y) == m_pack->GetTopCard())
	{
		return m_pack;
	}

	if (m_discard->GetCard(x, y) && m_discard->GetCard(x, y) == m_discard->GetTopCard())
	{
		return m_discard;
	}
	...
	for (i = 0; i < 10; i++)
	{
		if (m_bases[i]->GetCard(x, y) && m_bases[i]->GetCard(x, y) == m_bases[i]->GetTopCard())
		{
			return m_bases[i];
		}
	}
		...
		if (card)
		{
			...
			if (card->GetPipValue() == 1)
			{
				for (i = 0; i < 4; i++)
				{
					...
					if (m_topCard)
					{
						if (m_topCard->GetSuit() == card->GetSuit() && m_foundations[i + 4] != pile
			...
			for (i = 0; i < 10; i++)
			{
				if (m_bases[i]->AcceptCard(card) && m_bases[i] != pile && m_bases[i]->GetTopCard())
				{
					pile->RemoveTopCard(dc);
...
bool Game::CanYouGo(int x, int y)
{
	...
	if (pile && pile != m_pack)
	{
		...
		if (card)
		{
			...
			for (i = 0; i < 10; i++)
			{
				if (m_bases[i]->GetTopCard() && m_bases[i]->AcceptCard(card) && m_bases[i] != pile)
				{
					return true;
				}
			}
...
void Game::LButtonUp(wxDC& dc, int x, int y)
{
	if (m_srcPile)
	{
		...
		wxMemoryDC memoryDC;
		memoryDC.SelectObject(*m_bmap);
		dc.Blit(m_xPos, m_yPos, CardWidth, CardHeight, &memoryDC, 0, 0, wxCOPY);
...
void Game::MouseMove(wxDC& dc, int mx, int my)
{
	if (m_liftedCard)
	{
		...
		if (abs(dx) >= CardWidth || abs(dy) >= CardHeight)
		{
			...
			memoryDC.Blit(0, 0, CardWidth, CardHeight, &dc, m_xPos + dx, m_yPos + dy, wxCOPY);
		}
		else if (dx >= 0)
		{
			// dx >= 0
			...
			if (dy >= 0)
			{
				...
				memoryDC.Blit(0, 0, CardWidth - dx, CardHeight - dy, &memoryDC, dx, dy, wxCOPY);
				memoryDC.Blit(
					0,
					CardHeight - dy,
					CardWidth - dx,
			...
			else
			{
				...
				memoryDC.Blit(0, -dy, CardWidth - dx, CardHeight + dy, &memoryDC, dx, 0, wxCOPY);
				memoryDC.Blit(0, 0, CardWidth - dx, -dy, &dc, m_xPos + dx, m_yPos + dy, wxCOPY);
			}
			memoryDC.Blit(
				CardWidth - dx, 0, dx, CardHeight, &dc, m_xPos + CardWidth, m_yPos + dy, wxCOPY
			);
		}
		else
		{
			...
			if (dy >= 0)
			{
				...
				memoryDC.Blit(-dx, 0, CardWidth + dx, CardHeight - dy, &memoryDC, 0, dy, wxCOPY);
				memoryDC.Blit(
					-dx,
					CardHeight - dy,
					CardWidth + dx,
			...
			else
			{
				...
				memoryDC.Blit(-dx, -dy, CardWidth + dx, CardHeight + dy, &memoryDC, 0, 0, wxCOPY);
				memoryDC.Blit(-dx, 0, CardWidth + dx, -dy, &dc, m_xPos, m_yPos + dy, wxCOPY);
			}
			memoryDC.Blit(0, 0, -dx, CardHeight, &dc, m_xPos + dx, m_yPos + dy, wxCOPY);
		}
		m_xPos += dx;
		m_yPos += dy;
...
//----------------------------------------------//
Pack::Pack(FortyFrame* frame, Game* game, int x, int y)
	: Pile(frame, x, y, 0, 0)
{
	for (m_topCard = 0; m_topCard < NumCards; m_topCard++)
	{
		m_cards[m_topCard] = new Card(game, 1 + m_topCard / 2, facedown);
	}
...
void Pack::AddCard(Card* card)
{
	...
	{
		wxMessageBox(
			wxT("Pack::AddCard() Undo error"),
			wxT("Forty Thieves: Warning"),
...
//------------------------------------------------------//
Base::Base(FortyFrame* frame, int x, int y)
	: Pile(frame, x, y, 0, 12)
{
	m_topCard = -1;
}
...
//----------------------------------------------------------------//
Foundation::Foundation(FortyFrame* frame, int x, int y)
	: Pile(frame, x, y, 0, 0)
{
	m_topCard = -1;
}
...
//----------------------------------------------------//
Discard::Discard(FortyFrame* frame, int x, int y)
	: Pile(frame, x, y, 19, 0)
{
	m_topCard = -1;
}
...
Card* Discard::RemoveTopCard(wxDC& dc, int m_xOffset, int m_yOffset)
{
	...
	else
	{
		...
		GetTopCardPos(x, y);
		dc.SetClippingRegion(topX - m_xOffset, topY - m_yOffset, CardWidth, CardHeight);
