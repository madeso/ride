...
void BombsCanvas::DrawField(wxDC* dc, int xc1, int yc1, int xc2, int yc2)
{
	...
	for (x = xc1; x <= xc2; x++)
		for (y = yc1; y <= yc2; y++)
		{
			if (m_game->IsMarked(x, y))
			{
				...
				dc->DrawRectangle(
					x * m_cellWidth * X_UNIT,
					y * m_cellHeight * Y_UNIT,
					m_cellWidth * X_UNIT + 1,
					m_cellHeight * Y_UNIT + 1
				);
				buf = wxT("M");
				...
				if (! m_game->IsHidden(x, y) && m_game->IsBomb(x, y))
				{
					...
					dc->DrawLine(
						x * m_cellWidth * X_UNIT,
						y * m_cellHeight * Y_UNIT,
						(x + 1) * m_cellWidth * X_UNIT,
						(y + 1) * m_cellHeight * Y_UNIT
					);
					dc->DrawLine(
						x * m_cellWidth * X_UNIT,
						(y + 1) * m_cellHeight * Y_UNIT,
	...
	wxString msg;
	msg.Printf(
		wxT("%d bombs, %u marked, %d remaining cells"),
		m_game->GetNumBombs(),
		m_game->GetNumMarkedCells(),
...
void BombsCanvas::Uncover(int x, int y)
{
	...
	if (m_game->IsBomb(x, y) || hasWon)
	{
		...
		{
			wxMessageBox(
				wxT("Nice! You found all the bombs!"), wxT("wxWin Bombs"), wxOK | wxCENTRE
			);
		}
		else  // x,y is a bomb
		{
	...
	{
		int left = (x > 0) ? x - 1 : 0;
		int right = (x < gridWidth - 1) ? x + 1 : gridWidth - 1;
		int top = (y > 0) ? y - 1 : 0;
		int bottom = (y < gridHeight - 1) ? y + 1 : gridHeight - 1;

		int i, j;
		for (j = top; j <= bottom; j++)
			for (i = left; i <= right; i++)
				if ((i != x || j != y) && m_game->IsHidden(i, j) && ! m_game->IsMarked(i, j))
				{
					Uncover(i, j);
				}
	}
...
void BombsCanvas::OnMouseEvent(wxMouseEvent& event)
{
	...
	if (x < gridWidth && y < gridHeight)
	{
		if ((event.RightDown() || (event.LeftDown() && event.ShiftDown()))
			&& (m_game->IsHidden(x, y) || ! m_game->GetNumRemainingCells()))
		{
		...
		else if (event.LeftDown() && m_game->IsHidden(x, y) && ! m_game->IsMarked(x, y))
		{
			// store previous and current field
			...
			m_game->m_gridFocusX = x;
			m_game->m_gridFocusY = y;
			RefreshField(prevGridFocusX, prevGridFocusY, prevGridFocusX, prevGridFocusY);
			Uncover(x, y);
			return;
		}
	}
...
void BombsCanvas::OnChar(wxKeyEvent& event)
{
	...
	case WXK_RETURN:
		if ((prevGridFocusX == m_game->m_gridFocusX) && (prevGridFocusY == m_game->m_gridFocusY)
	...
	if ((prevGridFocusX != m_game->m_gridFocusX) || (prevGridFocusY != m_game->m_gridFocusY))
	{
		...
		RefreshField(prevGridFocusX, prevGridFocusY, prevGridFocusX, prevGridFocusY);
		RefreshField(
			m_game->m_gridFocusX, m_game->m_gridFocusY, m_game->m_gridFocusX, m_game->m_gridFocusY
		);
	}
}
