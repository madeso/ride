...
)
{
	...
	{
		return LoadResult::Error(wxString::Format("table %s doesn't contain %s", table_name, name));
	}
	...
	{
		return LoadResult::Error(
			wxString::Format("%s from table %s is not a valid value", name, table_name)
		);
	}
	*result = *value;
	...
	for (const auto base: array->get())
	{
		if (false == base->is_value())
		{
			return LoadResult::Error(wxString::Format(
				"value %d in array %s from table %s is not a value", index, name, table_name
			));
		}
		const auto value = GetValue<T>(base);
		...
		{
			return LoadResult::Error(wxString::Format(
				"value %d in array %s from table %s is not a valid value", index, name, table_name
			));
		}
...
void AddFeature(
	const std::vector<wxString>& deps, std::vector<wxString>* feats, const wxString& name
)
{
...
LoadResult Cargo::Load(const wxString& file)
{
	try
	{
		...
		if (package.get() == nullptr) return LoadResult::Error("No package in cargo file");
