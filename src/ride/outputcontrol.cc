...
OutputControl::OutputControl(MainWindow* main, OutputControlFunctions functions)
	: wxControl(main, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxBORDER_NONE)
	, main_(main)
...
const wxString ReplaceCmd(const ride::MachineSettings& machine, const wxString& cmd)
{
...
void OutputControl::ClearOutput()
{
	::ClearOutput(text_);
}

void OutputControl::WriteLine(const wxString& str)
{
	::WriteLine(text_, str);
}
...
void OutputControl::BindEvents()
{
	...
	Bind(
		wxEVT_MENU, &OutputControl::OnCopyThisCompilerMessage, this, ID_COPY_THIS_COMPILER_MESSAGE
	);
