...
public:
	...
	explicit IdleTimer(SingleRunner::Pimpl* p)
		: pimpl_(p)
	{
	}
...
public:
	...
	void OnTerminate(int pid, int status) override
	{
		...
		runner_->Append(wxString::Format(
			wxT("Process %u ('%s') terminated with exit code %d."), pid, cmd_.c_str(), status
		));
		runner_->Append("");
	...
	virtual bool HasInput()
	{
		...
		while (IsErrorAvailable())
		{
			...
			wxString line(
				reinterpret_cast<const char*>(buf.GetData()), wxConvUTF8, buf.GetDataLen()
			);
			runner_->Append(line);
			hasInput = true;
		}
