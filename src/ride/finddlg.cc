...
const int FindDlg::GetFlags()
{
	...
	{
	case 0: break;
	case 1: ret |= wxSTC_FIND_REGEXP; break;
	case 2: ret |= wxSTC_FIND_POSIX; break;
	default: assert(false && "invalid selection"); break;
	}
...
public:
	...
	explicit UndoRedo(wxStyledTextCtrl* s)
		: stc(s)
	{
		assert(stc);
		stc->BeginUndoAction();
...
)
{
	...
	while (true)
	{
		...
		if (find_action == FindAction::Replace)
		{
			...
			const int change
				= useRegex ? stc->ReplaceTargetRE(replaceText) : stc->ReplaceTarget(replaceText);
			assert(change > 0);
		...
		res->push_back(FindResult(
			file,
			stc->GetLine(start_line).Trim(true).Trim(false),
			start_line + 1,
			start_col + 1,
			end_line + 1,
			end_col + 1
...
)
{
	...
	for (auto res: results)
	{
		...
		const wxString mess = wxString::Format(
			...
			res.end_line,
			res.end_col,
			find_action == FindAction::Find ? "found" : "replaced",
			res.content
		);
